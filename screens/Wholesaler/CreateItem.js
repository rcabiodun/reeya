...
export default function CreateItemScreen({ navigation }) {
    ...
    const [selectedLocation, setSelectedLocation] = useState('Category');
    const [selectedBatch, setSelectedBatch] = useState('dozen');
    ...
    const uploadImage = async () => {
        ...
        fetch(CLOUDINARY_URL, {
            ...
            .then(async (res) => {
                ...
                await backend.create_Item({ title: ItemTitle, category: selectedLocation, image: data.secure_url, price: price,batch:selectedBatch }, addMessage, setmessages,setShowIndicator)
                console.log(data)
    ...
    return (
        <View style={styles.body} onLayout={async () => { await SplashScreen.hideAsync() }}>
            ...
            <View style={{ justifyContent: 'center', alignItems: "center", paddingVertical: 50 }}>
                <Text style={styles.heading}>Create your Item</Text>
            ...
            <Text style={{ fontFamily: "reg", color: color_scheme.grey, fontSize: 14, marginTop: 7 }}>Upload image of the item</Text>
